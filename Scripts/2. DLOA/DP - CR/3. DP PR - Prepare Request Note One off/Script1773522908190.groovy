...
// ===== helper: wait LOV popup overlay closed =====
def waitLovPopupClosed(int timeout = 20) {
	TestObject lovModal = new TestObject('lovModal')
	lovModal.addProperty("xpath", ConditionType.EQUALS,
		"//*[contains(@id,'lovPopup_modal') and contains(@class,'ui-widget-overlay')]"
	)

	if (WebUI.verifyElementPresent(lovModal, 1, FailureHandling.OPTIONAL)) {
		WebUI.waitForElementNotVisible(lovModal, timeout, FailureHandling.OPTIONAL)
	}
}

// ===== helper: submit once only =====
def clickSubmitOnce(TestObject obj, int timeout = 20) {
	wClickable(obj, timeout)
	WebUI.scrollToElement(obj, 1, FailureHandling.OPTIONAL)
	WebUI.click(obj)
	waitBlockUI(30)
...
// ===== main loop =====
int loopCount = 2   // loop 1 = item 1, loop 2 = item 2

for (int i = 1; i <= loopCount; i++) {
	WebUI.comment("Loop #" + i + " of " + loopCount)

	// make sure previous LOV popup is fully closed before next click
	waitBlockUI(20)
	waitLovPopupClosed(20)
	WebUI.delay(0.5)

	// click LOV link by item index
	TestObject lovLink = new TestObject("lovLink_" + i)
	lovLink.addProperty("xpath", ConditionType.EQUALS,
		"(//a[starts-with(@id,'_scDirectPurchaseTask_WAR_NGePportlet_:form:requestListDataTable:') " +
		"and contains(@id,':beforeSaveLov:beforeSaveLovlovLink')])[" + i + "]"
	)
	...
	// wait LOV popup content ready
	TestObject ddUsername = findTestObject('Object Repository/DLOA/6. Prepare Request Note/Dropdown Username')
	wVisible(ddUsername, 20)
	waitBlockUI(20)
	WebUI.delay(0.5)

	// Username dropdown
	selectDropdownByIndex(ddUsername, 1)
	waitBlockUI(20)
	WebUI.delay(0.5)

	// Prepared Username
	String preparedByText = PreparedBy
	t(findTestObject('Object Repository/DLOA/6. Prepare Request Note/Input Prepared Username'), preparedByText, 20)
	waitBlockUI(20)
	WebUI.delay(0.5)

	// Search button
	c(findTestObject('Object Repository/DLOA/6. Prepare Request Note/Click Search button'), 20)
	waitBlockUI(30)
	WebUI.delay(0.5)

	// Click Prepared Name in Table
	TestObject preparedName = findTestObject('Object Repository/DLOA/6. Prepare Request Note/Click Prepared Name in Table')
	wVisible(preparedName, 20)
	c(preparedName, 20)
	waitBlockUI(20)
	WebUI.delay(0.5)

	// Select button to close LOV popup
	c(findTestObject('Object Repository/DLOA/6. Prepare Request Note/Click Select Button'), 20)
	waitBlockUI(20)
	waitLovPopupClosed(20)
	WebUI.delay(0.5)
}
//approver
	...
	// Approver Group
	selectDropdownByIndex(findTestObject('Object Repository/DLOA/6. Prepare Request Note/Approver Group Dropdown'), 20)
	waitBlockUI(20)
	WebUI.delay(0.5)

	// Approver Name
	selectDropdownByIndex(findTestObject('Object Repository/DLOA/6. Prepare Request Note/Approver Name Dropdown'), 6)
	waitBlockUI(20)
	WebUI.delay(0.5)
...
String filePath = "C:\\Users\\hadishafiq\\Desktop\\PrepData\\DP_CR_DLOA_RN_Izzah_2026.xlsx"
