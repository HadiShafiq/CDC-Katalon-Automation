...
WebUI.delay(1)
/*=============================
 Looping For Product/Specification
===============================*/
...
for (int i = 1; i <= loopCount; i++) {
	...
	// =========================
	TestObject noRecordMsg = new TestObject("noRecordMsg_${i}")

	noRecordMsg.addProperty(
		"xpath",
		ConditionType.EQUALS,
		"//td[@colspan='6' and normalize-space()='No records found']"
	)

	if (WebUI.verifyElementPresent(noRecordMsg, 3, FailureHandling.OPTIONAL)) {

		WebUI.comment("No records found → click Action Item Product")

		c(findTestObject(
			'Object Repository/DLOA/4. DLOA - Requestioner/4.Product/Click Action Item Product'
		), 20)
		...
		WebUI.comment("Record exists and loop > 1 → click Action Item Product")
		...
		WebUI.comment("First loop and record exists → skip click Action Item Product")
	}
	...
	findTestObject('Object Repository/DLOA/4. DLOA - Requestioner/4.Product/Product Price Type'),
	Pricetype as int
	)
	...
	// =========================
	// Input Product Specification 1
	// =========================
	TestObject spec1Product = findTestObject(
		'Object Repository/DLOA/4. DLOA - Requestioner/4.Product/Product Specification 1'
	)

	WebUI.waitForElementVisible(spec1Product, 20)

	String existingValue = WebUI.getAttribute(spec1Product, "value")
	...
	if (existingValue == null || existingValue.trim().isEmpty()) {
		...
		WebUI.sendKeys(spec1Product, Keys.chord(Keys.TAB))
	...
	// Input Product UOM
	// =========================
	TestObject uomProduct = findTestObject(
		'Object Repository/DLOA/4. DLOA - Requestioner/4.Product/Input Product UOM'
	)

	wVisible(uomProduct, 20)

	WebUI.click(uomProduct)

	t(uomProduct, ProductUOM, 20)

	WebUI.delay(2)

	WebUI.sendKeys(uomProduct, Keys.chord(Keys.ENTER))
	...
	t(
		findTestObject(
			'Object Repository/DLOA/4. DLOA - Requestioner/4.Product/Product Quantity'
		),
		ProductQty,
		20
	)
	...
	// =========================
	// Input Product Delivery Period
	// =========================
	t(
		findTestObject(
			'Object Repository/DLOA/4. DLOA - Requestioner/4.Product/Product Delivery Period'
		),
		DeliveryPeriod,
		20
	)
	...
	// =========================
	// Click Action Item Product Specification
	// =========================
	TestObject specBtn = findTestObject(
		'Object Repository/DLOA/4. DLOA - Requestioner/4.Product/Product Action Item Click Specification'
	)
	...
	// Click Specification
	// =========================
	TestObject clickSpec = findTestObject(
		'Object Repository/DLOA/4. DLOA - Requestioner/4.Product/Product Click Specification'
	)
	...
	// =========================
	// Input Product Specification 2
	// =========================
	TestObject spec2Product = findTestObject(
		'Object Repository/DLOA/4. DLOA - Requestioner/4.Product/Input Specification 2 TextBox'
	)
...
int serviceLoopCount = 2

for (int s = 1; s <= serviceLoopCount; s++) {

	WebUI.comment("Service Loop #${s} of ${serviceLoopCount}")

	// =========================
	// Check "No records found"
	// =========================
	TestObject noRecordMsgService = new TestObject("noRecordMsg_Service_${s}")

	noRecordMsgService.addProperty(
		"xpath",
		ConditionType.EQUALS,
		"//td[@colspan='10' and normalize-space()='No records found']"
	)

	if (WebUI.verifyElementPresent(noRecordMsgService, 3, FailureHandling.OPTIONAL)) {

		WebUI.comment("No records found → click Action Item for Service")

		c(findTestObject(
			'Object Repository/DLOA/4. DLOA - Requestioner/1. General Information/Click Action Item for Service'
		), 20)
		...
		WebUI.comment("Record exists and loop > 1 → click Action Item for Service")
		...
		WebUI.comment("First loop and record exists → skip click Action Item for Service")
	}


	selectDropdownByIndex(
		findTestObject('Object Repository/DLOA/4. DLOA - Requestioner/2. Item List/Fulfilment Type Services'),
		Fulfilmenttype
	)

	TestObject spec1Service = findTestObject(
		'Object Repository/DLOA/4. DLOA - Requestioner/2. Item List/Service Input Specification 1 TextBox'
	)
	...
	if (existingValue == null || existingValue.trim().isEmpty()) {
		...
		String productSpec1WithLoopIndex = ServiceSpecification1 + s

		WebUI.setText(spec1Service, productSpec1WithLoopIndex)

		WebUI.sendKeys(spec1Service, Keys.chord(Keys.TAB))

		waitBlockUI(30)
		WebUI.delay(1)
	}

	TestObject uomService = findTestObject(
		'Object Repository/DLOA/4. DLOA - Requestioner/2. Item List/Input UOM'
	)
	...
	if (type == 2 || type == 3) {
		...
		TestObject freq = findTestObject(
			'Object Repository/DLOA/4. DLOA - Requestioner/2. Item List/Freq. Per UOM'
		)

		TestObject duration = findTestObject(
	...
	t(
		findTestObject('Object Repository/DLOA/4. DLOA - Requestioner/2. Item List/Quantity'),
		ServiceQty,
		20
	)
	...
	t(
		findTestObject('Object Repository/DLOA/4. DLOA - Requestioner/2. Item List/Service Period'),
		ServicePeriod,
		20
	)
	...
	TestObject clickSpec = findTestObject('Object Repository/DLOA/4. DLOA - Requestioner/5. Product_Plus_Services/Click Specification ProductPlusServices')
    ...
    String serviceSpec2WithLoopIndex = ServiceSpecification2 + s  // Add loop index to Specification Text
