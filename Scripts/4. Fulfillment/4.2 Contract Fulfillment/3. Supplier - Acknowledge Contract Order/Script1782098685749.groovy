 ...
 * Acknowledge
 * ========================= */
c(findTestObject('Object Repository/DP - Add To Cart/Contract List/Acknowledge'))
waitBlockUI(30)
WebUI.delay(1)
...
TestObject msgObj = new TestObject('msg_CO_saved')
...
// Wait until message text contains "CO"
String msg = ""
for (int i = 0; i < 2; i++) {
	msg = WebUI.getText(msgObj, FailureHandling.OPTIONAL)
	if (msg != null && msg.contains("CO")) break
	WebUI.delay(1)
}
...
// ===== 3) Extract CO number dynamically =====
def matcher = (msg =~ /(CO\d+)/)   // e.g. CO260000000001152
String coNo = matcher.find() ? matcher.group(1) : ""

if (coNo == "") {
	WebUI.takeScreenshot()
	assert false : "❌ CO number not found. Message was: " + msg
}
WebUI.comment("✅ Captured CO No: " + coNo)
...
if (Files.exists(path)) {
	...
	header.createCell(1).setCellValue("CO No")
...
row.createCell(1).setCellValue(coNo)
