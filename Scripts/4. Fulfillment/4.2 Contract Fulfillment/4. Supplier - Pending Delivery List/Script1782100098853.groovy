...
if (doNum == "") {
	WebUI.takeScreenshot()
	assert false : "❌ DO number not found. Message was: " + msg
}

WebUI.comment("✅ Captured DO No: " + doNum)
/* =========================
 * EXCEL APPEND 
 * ========================= */

String baseDir  = System.getProperty('user.home') + '/Desktop/PrepDataFileNumber'
String filePath = baseDir + '/Submit_Pending_Delivery_Contract_List.xlsx'
...
try {
	if (Files.exists(path)) {
		...
		header.createCell(1).setCellValue('DO No')
	...
	row.createCell(1).setCellValue(doNum)
