...
TestObject msgObj = new TestObject('msg_eP_saved')
msgObj.addProperty("xpath", ConditionType.EQUALS,
	"//span[contains(@class,'ui-messages-info-detail') and contains(.,'berikut')]"
)
...
for (int i = 0; i < 15; i++) {
	...
	if (msg != null && msg.contains("eP-")) break
	WebUI.delay(1)
}
...
def matcher = (msg =~ /(eP-[A-Z0-9]+)/)
String ePNumber = matcher.find() ? matcher.group(1) : ""

if (ePNumber == "") {
	WebUI.takeScreenshot()
	assert false : "❌ eP number not found. Message was: " + msg
}
WebUI.comment("✅ Captured eP No: " + ePNumber)


/* =========================
 * EXCEL APPEND
 * Purpose:
 * - append eP number and message into Excel file
 * ========================= */
String baseDir = System.getProperty("user.home") + "/Desktop/PrepDataFileNumber"
new File(baseDir).mkdirs()
String filePath = baseDir + "/BasicAccount_eP_Numbers.xlsx"
...
if (Files.exists(path)) {
	...
	header.createCell(1).setCellValue("eP No")
...
row.createCell(1).setCellValue(ePNumber)
