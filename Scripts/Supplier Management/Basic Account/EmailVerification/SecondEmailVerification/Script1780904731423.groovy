...
//Temp Password grabber variable = tempPassword

// Grab the paragraph
WebElement passElement = WebUI.findWebElement(
	findByXPath("//p[contains(.,'Temporary Password')]"), 10
)

// Full text will be: "Login ID: SMAUTOTEST1 Temporary Password: LoM_u65HrTa9."
String passFullText = passElement.getText()
println("Full text: " + passFullText)

// Split by lines since Login ID and Password are on separate lines via <br>
String[] lines = passFullText.split('\n')
String tempPassword = ''

for (String line : lines) {
	if (line.contains('Temporary Password')) {
		tempPassword = line.split('Temporary Password: ')[1].trim()
		break
	}
}

println("Temporary Password: " + tempPassword)

//Change password during Login
//Click HERE for 1st time login
c(findTestObject('Object Repository/EmailVerification/Yopmail/ClickHere1stTimeLogin'))
waitBlockUI(5)
